...
//
// or scripts/make-mac-icons.sh, which also runs iconutil to produce the .icns.
//
// Two outputs, because the two icons are read in completely different places:
//
//   assets/app-icon.iconset/
//       the Finder / About / permission-prompt icon, from assets/app-icon.png
...
let iconset = assets.appendingPathComponent("app-icon.iconset")
...
func writeMenuBarData(_ png: Data) {
    ...
    let swift = """
        //
        // Generated by scripts/make-app-icons.swift from assets/menu-icon.png.
...
print("app icon: \(iconsetSizes.count) sizes -> assets/app-icon.iconset")
