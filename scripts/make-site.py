#!/usr/bin/env python3
"""Regenerates the parts of site/ that must agree with the app.
...
makes scripts/generate-swift-tables.py exist. Two things are generated:

    site/assets/data.js         the symbol, script, fraction and operator
                                tables, plus every refusal reason
    site/index.html             the worked examples, between the marked
                                regions, each one produced by running the
                                real binary and capturing what it printed

Everything else in site/ is written by hand and never touched here.
...
def rows(cli, examples):
    ...
    for fragment, expected in examples:
        ...
        if actual != expected:
            sys.exit("make-site: %r now %ss, but the page says it should %s"
...
def counts():
    ...
    if not found:
        sys.exit("make-site: the check suite did not report a total")
    result["checks"] = "{:,}".format(int(found.group(1).replace(",", "")))

    summary = os.path.join(ROOT, "conformance", "windows-result.json")
    if os.path.exists(summary):
        ...
        sys.exit("make-site: no conformance/windows-result.json; "
...
def main():
    data = tables()
    write("// Generated by scripts/make-site.py from Sources/LaTeXUnicode.\n"
          "// Do not edit by hand: edit the tables and regenerate.\n"
          "const DATA = %s;\n" % json.dumps(data, ensure_ascii=False, separators=(",", ":")),
          "site", "assets", "data.js")
    print("site/assets/data.js: %d symbols, %d superscripts, %d subscripts, "
    ...
    for name, examples in EXAMPLES.items():
        begin = "<!-- BEGIN generated: %s (scripts/make-site.py) -->" % name
        end = "<!-- END generated: %s -->" % name
        marker = re.escape(begin) + ".*?" + re.escape(end)
        if not re.search(marker, page, re.S):
            sys.exit("make-site: site/index.html has no %s region" % name)
        filled = begin + "\n" + rows(cli, examples) + "\n        " + end
        page = re.sub(marker, lambda _: filled, page, flags=re.S)
        print("site/index.html: %d %s examples" % (len(examples), name))
    ...
    for name, value in counts().items():
        ...
        print("site/index.html: %s = %s" % (name, value))

    write(page, "site", "index.html")
