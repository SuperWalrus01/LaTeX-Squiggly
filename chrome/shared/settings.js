...
(() => {
  ...
  // googleDocs has its own switch because Docs is the one place the extension
  // cannot check the text before replacing it. See content/docs.js.
  const defaults = () => ({
    enabled: true,
    showNotices: true,
    googleDocs: true,
  ...
  async function load() {
    ...
    return {
      ...
      googleDocs: stored.googleDocs !== false,
