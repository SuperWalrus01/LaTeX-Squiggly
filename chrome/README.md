...
| `content/keyboard.js` | The key rules both of those share: the 64-character limit, what counts as a shortcut, and which sites a frame sits in. |
...
| `REVIEWING.md` | For the Web Store reviewer: where typing is read, what is stored, and the messages. The one note packaged into the zip. Keep it true when any of those change. |
