...
| Path | What it is |
|---|---|
| `manifest.json` | The extension's manifest. Its version must match `VERSION`, which `Tools/check_version.py` checks. |
| `engine/tables.js` | Generated by `Tools/generate_js_tables.py`. Do not edit by hand. |
...
| `icons/` | Generated by `Tools/make_chrome_icons.py` from the menu bar mark. |
...
Tools/generate_js_tables.py`, then run the conformance test.

## Packaging

```sh
Tools/package_chrome.sh
```
