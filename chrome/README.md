...
## Google Docs (experimental)

Off by default, and switched on in the settings page. The code is
...
**Not yet verified in Google Docs itself.** The browser test runs it against a
stand-in built the way Docs is understood to work, which proves the extension's
side. Docs' side needs a signed-in test by hand before this ships:

| Do this in a Google Doc, with the setting on | Expected |
|---|---|
| `\alpha` space | `α ` |
...
| Settings → turn Google Docs off, reload the doc, `\alpha` space | nothing converts |

If Docs ignores the key events, nothing converts and the command is left as
typed, with the space swallowed: say so in the report. If it takes the
Backspaces but not the keypresses, the command is deleted and nothing is typed:
turn the setting off at once and report it.

## What does not work

- **Google Sheets and Slides**, and Google Docs unless its experimental setting
  is on. The extension does nothing in any frame inside them, so it can never
  hand them a second copy of what was typed.
...
| `content/docs.js` | Google Docs mode, experimental and off by default. |
