...
def main():
    ...
    # ---- SymbolTable.swift ----
    body = "\n\n".join(emit_dict(t, r) for t, r in resolved_groups)
    with open(os.path.join(root, "Sources/LaTeXUnicode/SymbolTable.swift"), "w") as f:
        f.write(header)
        f.write("public enum SymbolTable {\n\n")
        f.write("    /// LaTeX command name (without the leading backslash) to its Unicode form.\n")
        f.write("    public static let symbols: [String: String] = [\n")
        f.write(body)
        f.write("\n    ]\n}\n")
